...
func openAPISpec(serverURL, environment string) map[string]any {
	return map[string]any{
		...
		"paths": map[string]any{
			...
			"/api/v1/emv/transactions": map[string]any{
				"post": map[string]any{
					...
					"responses":   map[string]any{"200": map[string]any{"description": "Transação processada com sucesso", "content": map[string]any{"application/json": map[string]any{"schema": map[string]any{"$ref": "#/components/schemas/ProcessedTransaction"}}}}, "400": map[string]any{"description": "Payload inválido"}, "422": map[string]any{"description": "Transação rejeitada por regra de negócio"}},
				},
				"get": map[string]any{
					"tags":        []string{"Transactions"},
		...
		"components": map[string]any{"schemas": map[string]any{
			...
			"ProcessedTransaction":      map[string]any{"type": "object", "properties": map[string]any{"transaction": map[string]any{"$ref": "#/components/schemas/Transaction"}, "authorization": map[string]any{"$ref": "#/components/schemas/AuthorizationResult"}, "status": map[string]any{"type": "string", "example": "approved"}}},
